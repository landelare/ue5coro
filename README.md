...
Unreal Engine 4 with a focus on gameplay logic, convenience, and providing
seamless integration with the engine.

> [!CAUTION]
> You're looking at a special, unsupported release.
...
* Why time slice on the game thread when you have multiple CPU cores eager to
  work?<br>
  Add `co_await MoveToThread(ENamedThreads::AnyThread);` to your function, and
  everything after that line will run on a worker thread.<br>
* Want to go back?
...
**Patch UE4 so that it supports C++20 or newer.**
Instructions to do so are not provided, the required changes depend on your
target compiler.

Do not use Git branches directly, only numbered UE5Coro releases.
...
If there is a `CppStandardVersion.Cpp20`, which isn't ordinarily available in
UE4, UE5Coro will use it; otherwise, it will use `CppStandardVersion.Latest`.

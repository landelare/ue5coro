...
co_await UE5Coro::WhenAll(Async, MoveTemp(Latent));
```
The code above might resume in an AsyncTask, or game thread Tick.
...
Async\:\:PlatformSeconds, and Async\:\:UntilPlatformTime resume
...
[^nomacro]: There is no `DECLARE_TS_DELEGATE`, but the delegates that it would
            define are supported anyway.
...
Like OnProcessRequestComplete(), it forces its caller back to the game thread.

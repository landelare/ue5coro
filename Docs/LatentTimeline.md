...
### TCoroutine<> Timeline(const UObject* WorldContextObject, float From, float To, float Duration, std::function<void(float)> Update, bool bRunWhenPaused = false)
### TCoroutine<> UnpausedTimeline(const UObject* WorldContextObject, float From, float To, float Duration, std::function<void(float)> Update, bool bRunWhenPaused = true)
### TCoroutine<> RealTimeline(const UObject* WorldContextObject, float From, float To, float Duration, std::function<void(float)> Update, bool bRunWhenPaused = true)
### TCoroutine<> AudioTimeline(const UObject* WorldContextObject, float From, float To, float Duration, std::function<void(float)> Update, bool bRunWhenPaused = false)
...
TCoroutine<> AActress::Dash(FVector From, FVector To, FForceLatentCoroutine = {})
{
    ...
    co_await Timeline(this, 0, 1, 0.5f, [&](float Alpha)
    {
...
> {
>     TCoroutine Coro = Timeline(this, 0, 1, 0.5f, [=, this](float Alpha)
>     {
>         FVector Location = FMath::Lerp(From, To, Alpha);
>         SetActorLocation(Location);
>     });
